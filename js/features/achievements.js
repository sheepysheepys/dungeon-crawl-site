...
(function () {
  ...
  /* ========== Modal + Confetti ========== */
  /* ---------- Modal + Confetti helpers (patched) ---------- */
  ...
  `;
    ...
    // Stronger, opinionated overlay layering
    const style = document.createElement('style');
    style.id = 'lootRevealStyles';
    style.textContent = `
    /* backdrop should sit above confetti and page */
    ...
    /* make any confetti canvas click-through and under the modal */
  ...
  `;
    if (!document.querySelector('#lootRevealStyles'))
      document.head.appendChild(style);

    const close = () => m.classList.add('hidden');

    // Close on buttons, backdrop click, and Esc
    ...
    // Prevent clicks inside the modal from closing or leaking
  ...
  // Normalize any existing confetti canvases (z-index & pointer events)
  ...
  async function fireConfetti() {
    ...
    // One quick burst; then normalize canvas layering
    window.confetti({
      particleCount: 70,
      spread: 70,
      startVelocity: 45,
